...
単一のフィールドで作成したインデックスでは昇順・降順関係なくそのインデックスが利用される。
...
単一フィールドのインデックスは並び順が昇順・降順に関わらず利用されるが、compound indexはフィールドの並び順によってインデックスが利用されるない場合がある。

次のインデックス `{ city: 1, zipCode: -1 }` を作成した場合、インデックスが利用されるのは指定通りの `.sort({ city: 1, zipCode: -1 })` と逆順の `.sort({city: -1,  zipCode: 1 })` となる。片方のフィールドのみがインデックスと逆順の場合は利用できない。
